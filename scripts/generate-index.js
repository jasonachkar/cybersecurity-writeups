...
const TYPE_LABELS = {
  ...
  "threat-intel": "🕵️ Threat Intelligence",
  tutorial: "📘 Tutorials",
  research: "🔬 Research",
...
/**
 * Recursively finds all markdown files under the given directory,
 * excluding readme.md files.
 * @param {string} dir
 * @returns {string[]}
 */
function findNewWriteupFiles(dir) {
  if (!fs.existsSync(dir)) return [];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      results.push(...findNewWriteupFiles(full));
      continue;
    }
...
function buildEntryLine(meta, relativeDirPath) {
  // Use directory name as fallback title, or file name without extension
  const isFile = relativeDirPath.endsWith(".md");
  const fallback = isFile
    ? path.basename(relativeDirPath, ".md").replace(/-/g, " ")
    : path.basename(relativeDirPath);
  // Title-case the fallback if it's a file name
  const safeFallback = isFile
    ? fallback.split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")
    : fallback;
  const safeTitle = meta.title || safeFallback;
  ...
  const metaPart =
    tagPreview || datePart || timePart
      ? ` — ${tagPreview}${tagSuffix}${datePart}${timePart}`.trim()
      : "";
...
function main() {
  ...
  // 1. Process legacy docs/ directories containing README.md
  ...
  // 2. Process new domain directories containing individual .md files
  const newDomains = ["cloud-security", "appsec", "devsecops", "threat-intel"];
  for (const domain of newDomains) {
    const domainDir = path.join(REPO_ROOT, domain);
    if (fs.existsSync(domainDir)) {
      const mdFiles = findNewWriteupFiles(domainDir);
      for (const filePath of mdFiles) {
        const raw = fs.readFileSync(filePath, "utf8");
        const { frontMatter } = parseFrontMatter(raw);
        const meta = normalizeMeta(frontMatter);
        // Force the type metadata to match the domain name if not explicitly set
        if (!frontMatter.type) {
          meta.type = domain;
        }

        const relPath = path.relative(REPO_ROOT, filePath);
        items.push({ meta, relDir: relPath });
      }
    }
  }
